...
## Instructions for this assignment can be found in this file, along with 
#some provided structure and some provided code.

## There are 3 parts to this assignment, each of which is described below.

## There are tests for each part, but the tests are NOT exhaustive, because
# you may each have different tweets, etc. Make sure you check the data 
#you get -- print it out, check the types, look in the database browser, 
#try out your SQL queries!

## We have provided setup code for you to use Tweepy, like we did for HW5
# and Project 2:
...
#Twitter user handle (e.g. "umsi" or "umich" or "Lin_Manuel" or 
	#"ColleenAtUMSI") and returns data that represents at least 20 
#tweets from that user's timeline.
...
# And then load all of those tweets you got from Twitter into a database 
#table called Tweets, with the following columns in each row:

## tweet_id - containing the unique id that belongs to each tweet
## author - containing the screen name of the user who posted the tweet 
#(note that even for RT'd tweets, it will be the person whose timeline it is)
## time_posted - containing the date/time value that represents when the 
#tweet was posted (note that this should be a TIMESTAMP column data type!)
## tweet_text - containing the text that goes with that tweet
## retweets - containing the number that represents how many times the 
...
#(so you can run the program over and over), with the correct (4) column 
#names and appropriate types for each.
# HINT: Remember that the time_posted column should be the TIMESTAMP data
# type!
cur.execute('DROP TABLE IF EXISTS Tweets')
table_spec ='CREATE TABLE IF NOT EXISTS '
table_spec +='Tweets (id INTEGER PRIMARY KEY, '
table_spec += 'tweet_id INTEGER, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweet_count INTEGER)'
cur.execute(table_spec)
#statement = "CREATE TABLE IF NOT EXISTS Tweets (tweet_id INTEGER, author TEXT, time_posted TIMESTAMP, tweet_text TEXT)"
#cur.execute(statement)
# Invoke the function you defined above to get a list that represents a 
#bunch of tweets from the UMSI timeline. Save those tweets in a variable 
#called umsi_tweets.
...
# that insert the data from each of the tweets in umsi_tweets into the 
#correct columns in each row of the Tweets database table.
...
tweet_text = []
retweet_count = []
#Tweets = ['None', 'id','screen_name','created_at','text']

#statement = 'INSERT INTO Tweets VALUES (?, ?, ?, ?, ?)'
#master = []
#for tweet in umsi_tweets:
#	#master.append('num_id')
#	master.append(tweet['id'])
#	master.append(tweet['user']['screen_name'])
#	master.append(tweet['created_at'])
#	master.append(tweet['text'])
#	master.append(tweet['retweet_count'])
	
for tweet in umsi_tweets:
	cur.execute('INSERT INTO Tweets VALUES (?, ?, ?, ?, ?, ?)', (None, tweet['id'], tweet['user']['screen_name'], tweet['created_at'], tweet['text'], tweet['retweet_count']))
conn.commit()

#(num_id, tweet_id, author, time_posted, tweet_text, retweet_count)
...
# statements to get data from the database, as directed. For each direction,
# write Python code that includes an SQL statement that will get the data 
#from your database. 
## You can verify whether your SQL statements work correctly in the SQLite
# browser! (And with the tests)


# Select from the database all of the TIMES the tweets you collected were 
#posted and fetch all the tuples that contain them in to the variable 
#tweet_posted_times.
...
#have been retweeted MORE than 2 times, and fetch them into the variable 
#more_than_2_rts.
query = "SELECT * FROM Tweets WHERE retweet_count > 2"
cur.execute(query)
...
#first_rt. Note that first_rt should contain a single string value, 
#not a tuple.
query = "SELECT tweet_text FROM Tweets WHERE instr(tweet_text, 'RT')"

cur.execute(query)
first_rt_tup = cur.fetchone()
for word in first_rt_tup:
	first_rt = word
...
# Define a function get_twitter_users that accepts a string as in 
#put and returns a SET of the _twitter screennames_ of each twitter 
#user who was mentioned in that string. 
def get_twitter_users(astring):
	a = set(re.findall('@[a-zA-Z0-9]*_*[a-zA-Z0-9]*.[a-zA-Z0-9]/', astring))
	return a


#   '@[a-zA-Z0-9]*_*[a-zA-Z0-9]*_*[a-zA-Z0-9]'
#		print("********")
# Note that the syntax for mentions in a tweet is that the username 
#is preceded by an "@" character, e.g. "@umsi" or "@aadl", and cannot 
...
#for you here... reminder: http://stackoverflow.com/questions/15340582/python-extract-pattern-matches
...
# Also note that the SET type is what this function should return, 
#NOT a list or tuple. We looked at very briefly at sets when we looked 
#at set comprehensions last week. In a Python 3 set, which is a special 
#data type, it's a lot like a combination of a list and a dictionary: 
#no key-value pairs, BUT each element in a set is by definition unique.
